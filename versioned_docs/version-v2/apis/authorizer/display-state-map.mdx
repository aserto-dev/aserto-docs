...
defines [decisions](policy#decision) called `visible` and `enabled`, which take on `true` / `false`
...
Aserto's [`decisiontree`](decisiontree) API makes it possible to compute these _display states_
