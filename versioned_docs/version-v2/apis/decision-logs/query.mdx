...
[decision log storage object retrieval](../../aserto-console/decision-logs#decision-logs-api-key), the `query` API returns data that is
...
See [schemas](schemas) for details of the contents of the logs returned by `query`.
