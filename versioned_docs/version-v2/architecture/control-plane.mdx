...
* **Organizations**: users can create an [organization](organization), invite other users to join, and assign them a role (viewer, member, admin, owner).
...
- [Connections](connections): connections to external systems such as identity providers and source code control systems
- [Policies](policy/policy-lifecycle): references to a set of authorization policy repositories
- [Directory](directory): the meta-directory that Aserto builds from the identity providers that it is connected to
- [Decision logs](decision-logs): a centralized log of the aggregated decisions that are made by authorizers
