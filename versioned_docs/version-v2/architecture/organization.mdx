...
[_connections_](connections), [_policies_](policy/policy-lifecycle), and the [_directory_](directory). You can think of it like a GitHub organization.
...
- [Connections](connections) - these are connections to external systems such as source code control systems and identity providers
- [Policies](policy/policy-lifecycle) - these are references to git repositories that store authorization policies
- [Directory](directory) - these are the objects that Aserto has synched from one or more connected identity providers
...
Learn more about how to [manage Aserto organizations](../aserto-console/manage-organizations) using the Aserto Console.
