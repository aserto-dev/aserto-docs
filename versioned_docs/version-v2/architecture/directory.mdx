...
any identity providers that has been [connected](connections) to the control plane.  Some identity
...
Policies can reference [user context](policy/overview#user-context). A calling application can
provide the [identity context](../apis/authorizer/identity-context) which is used to load that user context.
...
Authorization policies can access directory contents via a set of [built-ins](../apis/authorizer/built-ins).
