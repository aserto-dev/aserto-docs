...
is the low-level interface that talks to the Aserto [authorization API](../../apis/authorizer/authz).
...
Using an authorizer client we can call the [`Is()`](../../apis/authorizer/is) API to check if a user is authorized
...
We can similarly call the [`DecisionTree()`](../../apis/authorizer/decisiontree) and
[`Query()`](../../apis/authorizer/query) APIs.
