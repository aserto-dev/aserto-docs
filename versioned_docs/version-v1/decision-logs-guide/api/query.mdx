...
sidebar_label: query
title: The query API
description: Query for recent decisions
---

# decision_logs/query

The `query` API returns the last N decisions recorded for a particular policy. In contrast to
[decision log storage object retrieval](/docs/v1/decision-logs-guide/api/decision-logs), the `query` API returns data that is
...
See [schemas](/docs/v1/decision-logs-guide/schemas) for details of the contents of the logs returned by `query`.
