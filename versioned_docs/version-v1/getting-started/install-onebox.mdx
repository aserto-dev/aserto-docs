...
:::Note
For Windows (or if you want to download it as a zip file), refer to the [CLI installation page](/docs/v1/command-line-interface/aserto-cli/installation).
:::
...
Explore how the [PeopleFinder sample](/docs/v1/getting-started/peoplefinder/peoplefinder-overview) uses the Aserto express SDK and React SDK to inject authorization into the application’s API, and conditionally render UI elements based on the `visible` and `enabled` attributes of the policy.
