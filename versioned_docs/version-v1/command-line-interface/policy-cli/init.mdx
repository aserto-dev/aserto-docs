...
The `secret name` that was provided (by default, `TOKEN`) refers to a GitHub secret that contains a key (the Aserto Policy Registry key, or a GitHub PAT with the correct scopes) to push the built, tagged image to the policy registry.

For the Aserto Policy Registry, this should be the API key that can be found in the Console under the Aserto Policy Registry connection. This is the same API key that you used to login in using `policy login`.

For `ghcr.io`, the PAT needs to have the `repo` and `write:packages` scopes.
...
You can also use the `policy` CLI to manually [build](/docs/v1/command-line-interface/policy-cli/build), [tag](/docs/v1/command-line-interface/policy-cli/tag) and [push](/docs/v1/command-line-interface/policy-cli/push) the policy to a policy registry.
