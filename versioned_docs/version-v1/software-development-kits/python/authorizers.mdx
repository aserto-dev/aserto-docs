...
An `Authorizer` can be one of the [two types of supported authorizers](/docs/v1/authorizer-guide/overview/#hosted-authorizer-vs-edge-authorizer):
...
The Aserto [Authorizer](/docs/v1/overview/authorizer) exposes HTTPS-only endpoints. In order for the API client to communicate with the authorizer, TLS certificates must be verified.

In a development environment, the Aserto [Sidecar](/docs/v1/command-line-interface/aserto-cli/onebox) automatically creates a set of self-signed certificates and certificates of the CA (certificate authority) that signed them. It places them in a well-known location on the filesystem, defaulting to `$HOME/.config/aserto/sidecar/certs/`.
