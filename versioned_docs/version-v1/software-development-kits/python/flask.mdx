...
2.  `register_display_state_map`: application middleware which implements and exposes an endpoint for returning the [display state map](/docs/v1/authorizer-guide/display-state-map) for a service, based on its authorization policy.
...
- `authorizer` (_required_): An [`Authorizer`](/docs/v1/software-development-kits/python/authorizers) instance that describes the Authorizer service being used.
- `identity_provider` (_required_): A callable object which returns an `Identity` instance that represents a user.

  #### Example using the [aserto-idp](/docs/v1/software-development-kits/python/identity-providers) package to define an Auth0 identity provider
...
[**display state map**](/docs/v1/authorizer-guide/display-state-map) pattern to front-ends (e.g. the
[React SDK](/docs/v1/software-development-kits/javascript/react) and
[JavaScript SPA SDK](/docs/v1/software-development-kits/javascript/spa)).
