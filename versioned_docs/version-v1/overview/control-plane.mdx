...
* **Organizations**: users can create an [organization](/docs/v1/overview/organization), invite other users to join, and assign them a role (viewer, member, admin, owner).
...
- [Connections](/docs/v1/overview/connections): connections to external systems such as identity providers and source code control systems
- [Policies](/docs/v1/overview/policy-lifecycle): references to a set of authorization policy repositories
- [User directory](/docs/v1/overview/directory): the meta-directory that Aserto builds from the identity providers that it is connected to
- [Decision logs](/docs/v1/overview/decision-logs): a centralized log of the aggregated decisions that are made by authorizers
