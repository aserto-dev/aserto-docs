...
- **POLICY_REGISTRY** (policy registry): this is a connection between Aserto and a policy registry. Currently, only the Open Container Registry (OPCR) and the Aserto Container Registry (APCR) are supported.
...
Connections are created using the Aserto [Console](/docs/aserto-console/connections). Currently, a tenant
