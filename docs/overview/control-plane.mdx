...
The Aserto Control Plane provides a central management layer for Topaz authorizers. The control plane robust hosting layer for the Topaz Authorizer and Directory, and manages the lifecycle of policies, user context, and data using an eventing fabric that delivers any changes to all the Topaz authorizers in under a second.  In addition, it collects all the decision logs from Topaz instances and aggregates them into a composite decision log interface, which can be streamed or batched into a logging system
