...
The [Authorizer](/docs/overview/authorizer) is an open source authorization engine which uses the 
...
In addition to using OPA as its decision engine, the Authorizer includes an embedded database that 
caches all the data that's needed for authorization decisions. This data is sourced from the Aserto 
[Directory](/docs/overview/directory), which is inherits its design from Google's 
[Zanzibar](https://research.google/pubs/pub48190/) system. 

## Control Plane

The Aserto [Control Plane](/docs/overview/control-plane) manages the lifecycle of **policies**, 
