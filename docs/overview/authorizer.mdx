...
Aserto uses [Topaz](https://github.com/aserto-dev/topaz)  as its Authorizer, an open source project that provides a self-contained authorization engine, and includes an embedded implementation of a relationship directory. The Topaz Authorizer uses the [Open Policy Agent](https://www.openpolicyagent.org) (OPA) to
...
In addition, when you create an Aserto [organization](/docs/overview/organization), a Topaz Authorizer instance
hosted in Aserto's multi-tenant environment is provisioned for you. Since the authorizers are the same, using the hosted authorizer is the easiest way to start building applications that use Aserto.
