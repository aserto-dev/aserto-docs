...
Topaz expects a configuration file  the `config.yaml` file to be in `$HOME/.config/topaz/cfg/config.yaml`. The configuration file can be created using the `topaz configure` command, documented [here](/docs/command-line-interface/topaz-cli/configuration).
