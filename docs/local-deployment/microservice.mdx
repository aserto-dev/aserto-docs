...
Note that this command expects the `config.yaml` file to be in `$PWD/cfg/config.yaml`. The configuration file can be created using the `topaz configure` command, documented [here](/docs/command-line-interface/topaz-cli/configuration).
