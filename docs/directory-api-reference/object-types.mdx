...
All object type fields can be modified after creation except for the ID. The same [setObjetType](schema/mutations/set-object-type)
...
Since an object type is uniquely identified by its name, the `id` field in [setObjetType](schema/mutations/set-object-type)
