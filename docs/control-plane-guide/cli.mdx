...
Once logged in, the `aserto` CLI already has the Tenant ID and authorization information to communicate with the `relay` endpoint. You will need to determine the registered instance ID. You can either do this via the [console](/docs/control-plane-guide/api#instance-id), or via the CLI.
