...
```javascript
ws = new WebSocket("wss://.../api/v1/decision_logs/decisions/policy/<policy-id>",
  [
...
```javascript
ws = new WebSocket("wss://.../api/v1/decision_logs/decisions/policy/<policy-id>?since=<start date-time>",
  ...)
```
