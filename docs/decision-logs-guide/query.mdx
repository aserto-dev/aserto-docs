...
[decision log storage object retrieval](/docs/decision-logs-guide/decision-logs), the `query` API returns data that is
...
The response will resembe this:
...
See [schemas](/docs/decision-logs-guide) for details of the contents the logs returned by `query`. 