...
* `decision_time`: The UTC time when the decison call was executed by an authorizer.
...
* `path`: The path evaluated in by the authorizer call.
...
Each result of the [query](/docs/decision-logs-guide/query) API is a JSON object encoded as a string; once decoded, it
has a structure similar to the following example:
