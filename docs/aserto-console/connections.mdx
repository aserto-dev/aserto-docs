...
description: Managing Connections in the Aserto Console
---
...
:::note
Currently, Aserto supports **Auth0** and **Okta** as identity providers. More are on the way! 

In addition, the **"acmecorp"** identity provider is a sample IDP that is used throughout the Aserto quickstarts and 
samples.
:::

Click on the “Add a connection” button. You will be prompted to enter connection parameters that 
correspond to the values that the IDP requires. 

### Acmecorp IDP
For the **acmecorp** sample identity provider, only the name is required.

### Auth0 IDP
For **Auth0**, you will be asked for more parameters:
...
:::info
For more detailed instructions, refer to the [Connect IDP](/docs/getting-started-acmecorp/bring-your-own-idp/set-up-auth0) topic in the
