...
A relation type can optionally include named permissions. This provides a layer of indirection, which allows a relation (commonly referred to as a "role") to aggregate a set of permissions. 

Note that the relationship between relations and permissions is "many to many".
...
For example, the "owner", "editor", and "viewer" relations that we created for the "organization" object type can all contain the "read" permission. If a user has any of these relations to a particular organization instance, then they have the "read" permission.
