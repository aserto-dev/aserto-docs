...
We'll use the [objects](/docs/directory-api-reference/schema/queries/objects) query to find objects of type `user`:
...
:::note
The directory generates unique IDs for objects when they are inserted. Users in your directory will have different
IDs then the ones shown here. Be sure to copy the IDs of _your_ users when creating relations.
:::
...
We can verify that Rick has the new relations to the correct departments by inspecting his `user` object:
