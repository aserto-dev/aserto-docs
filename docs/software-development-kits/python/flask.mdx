...
- `authorizer` (_required_): An `Authorizer` instance that describes the Authorizer service being used

  #### Examples:

  ```py
  from aserto import HostedAuthorizer, EdgeAuthorizer

  # If using Aserto's hosted authorizer
  ...
  # If using a self-hosted (Edge) authorizer
...
- `identity_provider` (_required_): A callable object which returns an `Identity` instance that represents
  a user.
...
This SDK is open source and can be found on [GitHub](https://github.com/aserto-dev/aserto-python/tree/main/packages/flask-aserto).
