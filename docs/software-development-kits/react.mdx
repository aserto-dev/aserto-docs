...
[Express.js](/docs/software-development-kits/express#displayStateMap-middleware) to allow retrieving this 
...
- `setIdentity` can be used to set the identity to pass as an `identity` HTTP header.  It will override an `identity` header that is passed into `reload(headers)`.  This is the preferred way to send an identity to the [displayStateMap](/docs/software-development-kits/express#displayStateMap-middleware) API, which can be used to override the Authorization header by the displayStateMap middleware.
