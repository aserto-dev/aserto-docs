...
Click the `peoplefinder-rbac` policy in the Authorizer tab again, and open the `peoplefinder.POST.api.users.__id` module.
...
Back in the console, open the Authorizer tab and click on the `peoplefinder-rbac` instance. Ensure that the new policy is reflected in the Policy browser.
