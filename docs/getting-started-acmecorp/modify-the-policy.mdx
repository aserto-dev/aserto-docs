...
The expression `dir.is_manager_of` is part of a set of built-in Rego functions Aserto provides. In this case, it checks the attribute `manager` of the _selected_ user passed as part of the resource context (`input.resource.id`) and determines whether it is equal to the `user.id` of the logged in user.
