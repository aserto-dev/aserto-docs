...
So far, we were using a policy created by Aserto. Let's say our product manager has asked us to allow managers to be able to change their reports' title and department. We can do this by _modifying our policy_ and without making _any_ changes to our application code! We'll modify the policy we set up and learn how to update our changes in Aserto.

## Review the policy in the Aserto Console

Click the `peoplefinder-rbac` policy in the Policies tab again, and this time let's look at the `peoplefinder.POST.api.users.__id` module.
