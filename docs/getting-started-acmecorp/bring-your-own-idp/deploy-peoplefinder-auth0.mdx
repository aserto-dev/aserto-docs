...
title: Deploy PeopleFinder with Auth0 as its IDP
description: Deploy the PeopleFinder application with Auth0
---

# Deploy PeopleFinder bound to Auth0 as its IDP

Now that you have created the credentials needed, you’ll have to deploy a different version of the PeopleFinder application which uses Auth0 for authentication. Follow the deployment instructions in [this repo](https://github.com/aserto-demo/peoplefinder) to deploy the application to Netlify.
...
From here, please refer to the [Explore PeopleFinder](/docs/getting-started-acmecorp/explore-peoplefinder) topic to
explore how PeopleFinder uses Aserto for authorizing its operations.
