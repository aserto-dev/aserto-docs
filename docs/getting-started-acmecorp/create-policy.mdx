...
# Create a policy and an authorizer

The underpinning components of Aserto's authorization model are the [policy](/docs/overview/policy) and the authorizer.

**Policies** are authored, stored, and versioned as code in a `git` repository.

An **authorizer** is an instance of the decision engine's runtime associated with a tagged version of a policy (or a tag mask - more on this later). You can create multiple authorizers, each associated with different versions of a policy.
...
After creating your organization, you'll be redirected to the Policies tab, where you'll see the following:

<img src="/add-a-policy-repo.png" width="500px" />
...
After creating your policy, you will see it listed in the Policies tab:

![policies-list](/policies-list.png)
...
1. 0 - The _major_ version of the policy. All **minor and patch** versions under this version will be included in this policy. For example, if you create a 0.1 tag, any authorizer associated with the `0` tag will reflect the latest minor change.
2. 0.0 - The _minor_ version of the policy. All **patch** versions under this version will be included in this policy. For example, if you create a `0.0.1` tag, any authorizer associated with the `0.0` tag will reflect the latest patch change.
3. 0.0.0 - The _patch_ version of the policy. This tells the authorizer to use this particular version of the policy - and no other version will be used.
4. latest - Represents the latest version of the policy. An authorizer associated with this tag will always reflect the latest tag pushed to the repository.
...
First, click on the "Authorizer" tab. You'll see the following:

<img src="/add-authorizer.png" width="500px" />
...
Select `aserto-oci-policy-registry` from the dropdown.
...
Congratulations! You now have a clone of the `policy-peoplefinder-rbac` policy template in your GitHub account, and an associated Aserto Authorizer instanace. Later, you’re going to modify this policy repository to change the authorization policy of the PeopleFinder application. But first, let's set up demo users in your directory so that we can understand how user information is used in the authorization flow.
