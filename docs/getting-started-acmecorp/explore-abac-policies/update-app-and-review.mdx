...
To retrieve the new Policy ID, open the Authorizer tab in the Aserto console and click the `peoplefinder-abac` instance. Then, click on `Policy Settings`:

![policy-settings-abac](/getting-started-acmecorp/policy-settings-abac.png)

Update the `POLICY_ID` enviornment variable in Netlify to the Policy ID of the policy associated with the `peoplefinder-abac` instnace.
