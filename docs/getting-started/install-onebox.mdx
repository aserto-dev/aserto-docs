...
Explore how the [PeopleFinder sample](/getting-started/peoplefinder/peoplefinder-sample) uses the Aserto express SDK and React SDK to inject authorization into the application’s API, and conditionally render UI elements based on the `visible` and `enabled` attributes of the policy.

Deploy the [self-hosted](/getting-started/self-hosted) authorizer, directory, and a web console to your own kubernetes cluster.