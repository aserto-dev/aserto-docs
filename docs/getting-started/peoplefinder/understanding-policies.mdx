...
description: Deeper dive into the structure and content of Aserto policies
---
...
A Policy is a collection of decisions that govern access to the resources within an application. At runtime, Aserto uses Policies to determine whether or not to allow a caller to access resources as 

## How Aserto Policies are authored and deployed

Aserto Policies contain a collection of Policy Modules which are authored in a declarative language called [Rego](https://www.openpolicyagent.org/docs/latest/policy-language/). For each API that needs to be authorized, there should be a corresponding Policy Module. Each module contains one or more decisions All the modules within your application are packaged into a Policy Image which can then be deployed to a Policy Instance, which uses an [Aserto Authorizer](/docs/overview/authorizer) behind the scenes at runtime.
...
* Allowed - Whether the specific API call should1 be allowed.

Having Aserto handle these concerns, makes the app easier to maintain as the policy behavior can be changed without having to redeploy the app. PeopleFinder users Aserto's [SDKs](/docs/software-development-kits/overview) in the browser and on the server in order to invoke its Policy.

# PeopleFinder Policies in Aserto

Let's take a look at how PeopleFinder's policies are configured in Aserto. Head over the Aserto Console a click on the Policies. You should see the PeopleFinder policy in the list. Click on it.
...
PeopleFinder has 6 Policy Modules shown here which are using an RBAC model. Each module is named using a convention that matches up against an API route in the application. This quick table shows the module name and a sample HTTP request so you can see the convention.

|Module                                |Example Request                                    |
|--------------------------------------|---------------------------------------------------|
|peoplefinder.DELETE.api.users.__id    | DELETE /users/1                                   |
|peoplefinder.GET.api.users            | GET /users                                        |
|peoplefinder.GET.api.users.__id       | GET /users/1                                      |
|peoplefinder.POST.api.users           | POST /users                                       |
|peoplefinder.POST.api.users.__id      | POST /users/1                                     |
|peoplefinder.PUT.api.users.__id       | PUT /users/1                                      |

In viewer you can select a module on the left and see the policy [Rego](https://www.openpolicyagent.org/docs/latest/policy-language/) code. Each of the PeopleFinder Policy Modules has been documented so you can see how they work.
...
This table shows the different decisions rules that are listed
...
Looking at the Rego code, it requires the logged-in user to be an admin in ordre to update the employee's department and title.
...
Using the Evaluator allows you to conveniently test you policy instantly within the console. In this case, let's test the previous module and see if it indeed only allows admins to update their department or title. First we'll test out Euan trying to update himself.
...
* For Resource Context, put ```{ "id": "dfdadc39-7335-404d-af66-c77cf13a15f8" }``` which is Euan's id. This provides the resource that the caller is accessing.
...
Great, we can see the result is `true`, proving that admins can access and our policy module is working correctly.

## Summary

You've just learned about Aserto Policies, how they are authored, and how they function. You've explored PeopleFinder's policy in the console and have seen how to use the Evaluator to test out your policies to ensure they are working as expected.
