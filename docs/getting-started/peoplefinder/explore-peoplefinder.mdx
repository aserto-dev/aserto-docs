...
Users logging in to PeopleFinder have different roles either 'viewer', 'editor', or 'admin'. PeopleFinder's Aserto policies determines what the logged-in user is allowed to do based on their role rather than it being hard coded within the application.
...
Let's see how this works. Notice the "Edit" button is enabled, while the "Update" button is disabled. This is because Euan (the current logged-in user) has the "Viewer" role so he can edit his own phone number. We can verify his role by looking at Euan's details from Aserto's directory. To do this, press the "Show Detail" button. Notice that "viewer" is listed underneath his roles.

![peoplefinder-euan-detail](/peoplefinder-euan-detail.png)

Press "Hide Detail" to close Euan's details and then pess the "Edit" button and modify the phone number, then click Save. 
...
Let's go one step further and see what happens when Kris views Euan. Go select "Kris" in the dropdown selector.
...
Type "kris" into the search bar to bring up "Kris Johson"
