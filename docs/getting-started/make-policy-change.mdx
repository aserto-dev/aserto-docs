...
The Aserto console shows the current loaded policy version attached to a policy instance for your tenant. If you click the `peoplefinder` policy instance in the Policies tab, you will see the current set of policy modules.
...
Now commit the changes, and create and push a new tag, so that the Aserto CI/CD workflow for policy changes kicks in, creates a new policy bundle based on the new tag, and makes it available to the policy instance:
