...
* A domain model for Slack, including `folder` and `doc` object types, `owner`, `editor`, and `viewer` relationships, and `can_read`, `can_write`, `can_share`, and `can_delete` permissions.
...
* A back-end API that uses the `gdrve` policy for authorization, implemented in several languages.
