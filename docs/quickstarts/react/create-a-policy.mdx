...
To set up an policy instance, head to the Policies tab and click the "Create an instance" button:
...
From the "Tag list" dropdown, select "latest". This sets up the policy instance to always pick up the _latest_ version of the policy. While this is going to be very useful while we are developing the policy - we would probably choose to lock the policy instance to a specific tag in a production enviornment.
