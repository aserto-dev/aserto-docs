...
In `index.js` add the following dependency after the `oidc-react` dependency:
...
This effect will trigger a call to the Display State Map endpoint that is set up on our server, and retrieve the `visible` and `enabled` decisions from the authorizer for our policy. Note that we specify the `GET` method and the `/api/protected` path in our call to `getDisplayState` since there could be multiple policies we may to get the display state for - this call will specifically check the policy `asertodemo.GET.api.protected`.

Outside of the effect, we’ll add some error handling - if the Aserto SDK ran into an error, we’ll display it and not render anything else. Add the following right after the effect we just added:
            ...
            {message && message === 401 && (
              <>
                ...
                <div className="message">
                  No access to sensitive information (Unauthorized)
                </div>
              </>
            )}
            {message && message === 403 && (
              <>
                <div className="sad-lottie"></div>
                <div className="message">
                  No access to sensitive information (Forbidden)
                </div>
              </>
            )}
          </div>
