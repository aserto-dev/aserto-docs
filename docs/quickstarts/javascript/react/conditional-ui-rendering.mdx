...
This effect will trigger a call to the Display State Map endpoint that is set up on our server, and retrieve the `visible` and `enabled` decisions from the authorizer for the policy defined as our root policy in our `.env` variables - `asertodemo`. More specifically, it'll get the package `asertodemo.GET.api.protected`.
