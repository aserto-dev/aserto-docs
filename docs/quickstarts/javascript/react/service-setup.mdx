...
To get started, let's create a new folder called service under the React application folder and run:
...
In the `service` folder, Create a file called index.js - that will be our server. To this file, we'll add the following dependencies:
...
app.get("/api/protected", checkJwt, function (req, res) {
  //send the response
  res.json({ secretMessage: "Very sensitive information presented here" });
});
...
Awesome! our service is now listening on port 8080 and we have a protected endpoint. In the [next section](update-the-application) we'll test this endpoint by updating our application to send a JWT token.
