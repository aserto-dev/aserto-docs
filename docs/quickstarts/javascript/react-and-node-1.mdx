...
Users in this identity provider have properties and roles associated with them. In this case, among other roles, Euan has the role of a “viewer”. If you search for the user Kris Johnson and inspect her associated JSON object, you’ll see she has the role of “admin”. Later in this tutorial we will leverage this information to allow the authorizer to make a decision as to which user will have access to a piece of sensitive information. Before we do that, we have to set up our React application as well as the Express.js application.
...
Lastly, we set up a protected route which will use the `checkMiddleware`:
...
Until now, we dealt only with authentication of users in our application. Now, let’s discuss how to set up an authorization model which will enforce some limitation on user access to our protected resource.
...
In this chapter, we created a simple React application and a simple Express.js application, using OICD as an identity provider. We then set up an Aserto Policy to limit user access to a protected resource using the identity context passed from the React Application. Finally, we saw how to make changes to the policy without requiring any changes to the application code.

In the next chapter, we’ll learn how to conditionally render UI components based on policy decisions.
