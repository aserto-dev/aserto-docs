...
opa:
  ...
  config:
    services:
      aserto-discovery:
        url: https://discovery.aserto.com/api/v2/discovery
        credentials:
          bearer:
    ...
    discovery:
      name: 'opa'
      prefix: '<POLICY_NAME>/<INSTANCE_LABEL>'
...
```sh
curl --request GET \
     --url https://discovery.aserto.com/api/v2/discovery/<POLICY_NAME>/<INSTANCE_LABEL>/opa \
    ...
    "discovery": {
      "bundles": {
        "<POLICY_ID>": {
          ...
          "resource": "<TENANT>/<POLICY_NAME>:<INSTANCE_LABEL>",
