...
The Aserto CLI can be used to configure the Topaz instance to use the control plane by adding an option to the `configure` sub-command.

```
aserto x configure <policy-name> --edge-authorizer <edge-authorizer-connection-id>
```

This will install the client certificates and configure the authorizer to connect to the control plane using them. The `edge-authorizer-connection-id` can be acquired from the console, or using the Aserto CLI:
