...
To use an edge authorizer, you'll first need to create an edge authorizer connection. Head to the Connections tab, and create a new connection by clicking the "Add connection" button. Select the "Edge Authorizer" provider then provide a name and a display name for your the edge authorizer connection.
...
The Aserto CLI can be used to configure a Onebox to use the control plane by adding an option to the `configure` sub-command.
...
To configure a sidecar authorizer, it is first necessary to get the client certificates for the desired edge authorizer connection. This can be done with the Aserto CLI:
