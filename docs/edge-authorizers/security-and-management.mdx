...
This will return a JSON object with data about the client certificate. The `certificate` field contains the certificate
in PEM format and the `private_key` field contains the private key, also in PEM format. The contents of these fields
should be saved as two files, for example `~/.config/aserto/sidecar.crt` and `~/.config/aserto/sidecar.key`.
