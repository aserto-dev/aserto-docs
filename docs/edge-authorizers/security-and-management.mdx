...
The Aserto CLI can then be used to [configure](/docs/command-line-interface/aserto-cli/onebox#configuring) the Topaz instance to use the Control Plane by adding an option to the `configure` sub-command. This will install the client certificates and configure the authorizer to connect to the Control Plane using them. 
