...
:::danger Note
The `onebox` authorizer flavor has been deprecated. Please use the `sidecar`.
:::
...
Or by using the `docker` CLI selecting the correct image (`ghcr.io/aserto-dev/sidecar:latest`):
