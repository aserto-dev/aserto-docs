...
The decision stream is a near real-time stream of decison events. Events are typically delivered in under one minute
and are bufferd for replay. The `decisions` command can be used to plug into the stream and output it to `stdout`.

```
aserto decisions stream <policy-id> [--since <time in RFC3339 format>]
```

The CLI will remain open, outputting events to `stdout`. `since` specifies the event time to start streaming from in
